...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setContexts(0, 1, 2)
        .setDescription("Spam only 1 User on the Realm")
    ...
    async execute(interaction) {
        ...
        const namespoof = interaction.options.getString('namespoof_name') || "§c Frosted"
                ...
                try {
                    ...
                    setTimeout(() => {
                        ...
                        for (let i = 0; i < packetCount; i++) {
                            ...
                            let message = message1 + (bypass ? ` ${genrandomstring(8)}` : '');
                            let command;
                            if (rainbow) {
                                        command = `/tell ${escapedTarget} ${colorizeText(message, true)} ${rainbowLink}`;
                            } else {
                                command = `/tell ${escapedTarget}  ${message}`;
                                }
                            }
